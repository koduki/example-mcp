...
export function registerPrompts(server: Server): void {
  const simplePrompt = new PromptBuilder()
    ...
    .setTemplate(
      new PromptTemplate([
        ...
        {
          role: 'assistant',
          content: 'I would be happy to help you with anything you need. What can I assist you with today?'
        }
      ])
    )
    .build();
  ...
  const complexPrompt = new PromptBuilder()
    ...
    .setTemplate((args: Record<string, unknown> | undefined) => {
  ...
  server.registerPrompt(simplePrompt);
  server.registerPrompt(complexPrompt);
}
